...
const props = defineProps({
  ...
  planeWidth: { type: Number, required: false, default: 320 },
  planeHeight: { type: Number, required: false, default: 320 },
  distortion: { type: Number, required: false, default: 3 },
  scrollEase: { type: Number, required: false, default: 0.01 },
  cameraFov: { type: Number, required: false, default: 45 },
  cameraZ: { type: Number, required: false, default: 20 },
...
function AutoBind(self, { include, exclude } = {}) {
  ...
  const filter = (key) => {
    const match = (pattern) => typeof pattern === "string" ? key === pattern : pattern.test(key.toString());
...
class Canvas {
  ...
  onWheel(e) {
    this.scroll.target += e.deltaY * 5e-3;
  }
  update() {

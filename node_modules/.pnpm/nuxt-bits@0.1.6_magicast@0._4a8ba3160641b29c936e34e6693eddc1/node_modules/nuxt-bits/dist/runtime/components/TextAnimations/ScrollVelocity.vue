		...
		>
			...
			>
				<span
					...
					:key="spanIndex"
					:ref="spanIndex === 1 ? (el) => setCopyRef(el, index) : void 0"
...
const props = defineProps({
  scrollContainerRef: { type: null, required: false },
  texts: { type: Array, required: false, default: () => [] },
  velocity: { type: Number, required: false, default: 100 },
  className: { type: String, required: false, default: "" },
  damping: { type: Number, required: false, default: 50 },
  stiffness: { type: Number, required: false, default: 400 },
  velocityMapping: { type: Object, required: false, default: () => ({ input: [0, 1e3], output: [0, 5] }) },
  parallaxClassName: { type: String, required: false, default: "" },
  scrollerClassName: { type: String, required: false, default: "" },
  parallaxStyle: { type: Object, required: false, default: () => ({}) },
  scrollerStyle: { type: Object, required: false, default: () => ({}) }
});
...
const updateSmoothVelocity = () => {
  const dampingFactor = props.damping / 1e3;
  const stiffnessFactor = props.stiffness / 1e3;
...
const animate = (currentTime) => {
  ...
  props.texts.forEach((_, index) => {
    ...
    let moveBy = (directionFactors.value[index] || 1) * baseVelocity * (delta / 1e3);
...
const updateScrollVelocity = () => {
  ...
  if (timeDelta > 0) {
    ...
    scrollVelocity.value = scrollDelta / timeDelta * 1e3;
  }
  lastScrollY = currentScrollY;
};
onMounted(async () => {
  await nextTick();
  baseX.value = Array.from({ length: props.texts.length }).fill(0);
  copyWidths.value = Array.from({ length: props.texts.length }).fill(0);
  calculatedCopies.value = Array.from({ length: props.texts.length }).fill(15);
  directionFactors.value = Array.from({ length: props.texts.length }).fill(1);
  setTimeout(() => {
    updateWidths();
  }, 100);
  updateWidths();

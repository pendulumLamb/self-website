...
const props = defineProps({
  effectOptions: { type: Object, required: false, default: () => ({}) }
});
const hyperspeedContainer = ref();
let appRef = null;
...
const distortions = {
  ...
  turbulentDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const getX = (p) => Math.cos(Math.PI * p * uFreq.x + time) * uAmp.x + Math.cos(Math.PI * p * uFreq.y + time * (uFreq.y / uFreq.x)) ** 2 * uAmp.y;
      const getY = (p) => -nsin(Math.PI * p * uFreq.z + time) * uAmp.z - nsin(Math.PI * p * uFreq.w + time / (uFreq.z / uFreq.w)) ** 5 * uAmp.w;
      const distortion = new THREE.Vector3(
  ...
  deepDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const getY = (p) => (p * uPowY.x) ** uPowY.y + Math.sin(p * Math.PI * uFreq.y + time) * uAmp.y;
      const distortion = new THREE.Vector3(
...
class App {
  ...
  constructor(container, options) {
    ...
    this.camera = new THREE.PerspectiveCamera(options.fov, container.offsetWidth / container.offsetHeight, 0.1, 1e4);
...
const initHyperspeed = async () => {
  if (appRef) {
    appRef.dispose();
    const container2 = hyperspeedContainer.value;
    if (container2) {
...
<style scoped>
.hyperspeed-container{@apply size-full relative overflow-hidden}.hyperspeed-container :deep(canvas){@apply !size-full !block}
</style>

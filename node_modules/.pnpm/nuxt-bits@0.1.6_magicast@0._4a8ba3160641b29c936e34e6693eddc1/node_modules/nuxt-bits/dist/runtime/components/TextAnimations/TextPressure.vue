<template>
	<div ref="containerRef" class="relative size-full overflow-hidden bg-transparent">
		<h1 ref="titleRef" :class="`text-pressure-title ${className} ${flex ? 'flex justify-between' : ''} ${stroke ? 'stroke' : ''} uppercase text-center`" :style="titleStyle">
			<span v-for="(char, i) in chars" :key="i" :ref="(el) => spansRef[i] = el" :data-char="char" class="inline-block">
				{{ char }}
			</span>
		</h1>
	</div>
...
const props = defineProps({
  text: { type: String, required: false, default: "Compressa" },
  fontFamily: { type: String, required: false, default: "Compressa VF" },
  fontUrl: { type: String, required: false, default: "https://res.cloudinary.com/dr6lvwubh/raw/upload/v1529908256/CompressaPRO-GX.woff2" },
  width: { type: Boolean, required: false, default: true },
  weight: { type: Boolean, required: false, default: true },
  italic: { type: Boolean, required: false, default: true },
  alpha: { type: Boolean, required: false, default: false },
  flex: { type: Boolean, required: false, default: true },
  ...
  strokeColor: { type: String, required: false, default: "#FF0000" },
  strokeWidth: { type: Number, required: false, default: 2 },
  className: { type: String, required: false, default: "" },
  minFontSize: { type: Number, required: false, default: 24 }
});
...
const titleStyle = computed(() => ({
  ...
  margin: 0,
  fontWeight: 100,
  color: props.stroke ? void 0 : props.textColor
}));
onMounted(() => {
  const styleElement = document.createElement("style");
  styleElement.textContent = dynamicStyles.value;

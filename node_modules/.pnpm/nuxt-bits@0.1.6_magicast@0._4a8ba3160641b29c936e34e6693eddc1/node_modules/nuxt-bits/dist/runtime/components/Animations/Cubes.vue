...
const props = defineProps({
  gridSize: { type: Number, required: false, default: 10 },
  cubeSize: { type: Number, required: false },
  maxAngle: { type: Number, required: false, default: 45 },
  radius: { type: Number, required: false, default: 3 },
  easing: { type: null, required: false, default: "power3.out" },
  duration: { type: Object, required: false, default: () => ({ enter: 0.3, leave: 0.6 }) },
  cellGap: { type: [Number, Object], required: false },
  borderStyle: { type: String, required: false, default: "1px solid #fff" },
  faceColor: { type: String, required: false, default: "#0b0b0b" },
  shadow: { type: [Boolean, String], required: false, default: false },
  autoAnimate: { type: Boolean, required: false, default: true },
  rippleOnClick: { type: Boolean, required: false, default: true },
  rippleColor: { type: String, required: false, default: "#fff" },
  rippleSpeed: { type: Number, required: false, default: 2 }
});
const sceneRef = ref(null);
const rafRef = ref(null);
const idleTimerRef = ref(null);
const userActiveRef = ref(false);
const simPosRef = ref({ x: 0, y: 0 });
const simTargetRef = ref({ x: 0, y: 0 });
const simRAFRef = ref(null);
const colGap = computed(() => {
  return typeof props.cellGap === "number" ? `${props.cellGap}px` : props.cellGap?.col !== void 0 ? `${props.cellGap.col}px` : "5%";
});
const rowGap = computed(() => {
  return typeof props.cellGap === "number" ? `${props.cellGap}px` : props.cellGap?.row !== void 0 ? `${props.cellGap.row}px` : "5%";
});
...
const resetAll = () => {
  if (!sceneRef.value) return;
  sceneRef.value.querySelectorAll(".cube").forEach(
    (cube) => gsap.to(cube, {
      duration: leaveDur.value,
      rotateX: 0,
      rotateY: 0,

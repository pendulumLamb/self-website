...
const props = defineProps({
  ...
  gridColor: { type: String, required: false, default: "#27FF64" },
  rippleIntensity: { type: Number, required: false, default: 0.05 },
  gridSize: { type: Number, required: false, default: 10 },
  gridThickness: { type: Number, required: false, default: 15 },
  fadeDistance: { type: Number, required: false, default: 1.5 },
  vignetteStrength: { type: Number, required: false, default: 2 },
  glowIntensity: { type: Number, required: false, default: 0.1 },
  opacity: { type: Number, required: false, default: 1 },
  gridRotation: { type: Number, required: false, default: 0 },
  mouseInteraction: { type: Boolean, required: false, default: true },
  mouseInteractionRadius: { type: Number, required: false, default: 1 }
});
const hexToRgb = (hex) => {
  const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);
  return result ? [Number.parseInt(String(result[1]), 16) / 255, Number.parseInt(String(result[2]), 16) / 255, Number.parseInt(String(result[3]), 16) / 255] : [1, 1, 1];
};
const uniforms = {
  iTime: { value: 0 },
...
void main() {
  ...
  const handleMouseEnter = () => {
    ...
    mouseInfluenceRef.value = 1;
  };
  const handleMouseLeave = () => {
    if (!props.mouseInteraction) return;
    mouseInfluenceRef.value = 0;
  };

...
const props = defineProps({
  imageData: { type: null, required: true },
...
function createShader(gl2, sourceCode, type) {
...
function getUniforms(program, gl2) {
  const uniformsObj = {};
  const uniformCount = gl2.getProgramParameter(program, gl2.ACTIVE_UNIFORMS);
  for (let i = 0; i < uniformCount; i++) {
    const uniformName = gl2.getActiveUniform(program, i)?.name;
    if (!uniformName) continue;
    uniformsObj[uniformName] = gl2.getUniformLocation(program, uniformName);
  }
  return uniformsObj;
}
function initShader() {

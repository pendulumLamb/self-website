...
const props = defineProps({
  ...
  spinDuration: { type: Number, required: false, default: 20 },
  onHover: { type: String, required: false, default: "speedUp" },
  className: { type: String, required: false, default: "text-white" }
});
...
const animate = () => {
  const now = Date.now();
  const deltaTime = (now - lastTime.value) / 1e3;
  lastTime.value = now;
